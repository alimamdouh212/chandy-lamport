...
type Simulator struct {
	...
	snaps            map[int]*SnapshotState
	conmpletednumper map[int]int
	resluts          map[int]*(chan *SnapshotState)
}
...
func (sim *Simulator) StartSnapshot(serverId string) {
	...
	sim.servers[serverId].StartSnapshot(snapshotId, false)
	f := make(chan *SnapshotState)
	sim.resluts[snapshotId] = &f
...
func (sim *Simulator) NotifySnapshotComplete(serverId string, snapshotId int) {
	...
	// TODO: IMPLEMENT ME
	result := sim.servers[serverId].myactives[snapshotId]
	sim.snaps[snapshotId].tokens[serverId] = result.myservertoken
	for i, _ := range result.messages {
		sim.snaps[snapshotId].messages = append(sim.snaps[snapshotId].messages, &result.messages[i])
	}
	sim.conmpletednumper[snapshotId]++
	if sim.conmpletednumper[snapshotId] == len(sim.servers) {
		go provide(sim, snapshotId)
	}
}
func provide(sim *Simulator, snapshotId int) {

	t := sim.resluts[snapshotId]
